			...
			if (string_char_at(special, 0) != "s") {
					...
					if (ds_list_size(options) > 0) {
						...
						if (type == 3) {
							...
							if (special == "debt collector") {
								repeat(effect[3]) {
									instance_create_layer(x, y, "Projectiles", projectile_obj, {damage : damage * buffs[1] * buffs[4],
																								speed : projSpeed,
																								aoe : aoe,
																								special : special,
																								spread : spread,
				...
				if (special == "s commander") {
					...
					if (enemySpawner_obj.enemies > 0 and not firing) {
						...
						for (var i = 0; i < 6; i++) {
							instance_create_layer(path_get_x(minTarget.path_index, realMinPos + i * increment), path_get_y(minTarget.path_index, realMinPos + i * increment), "Projectiles", projectile_obj, {damage : damage * buffs[1] * buffs[4],
																								speed : projSpeed,
		...
		if (mouse_check_button_pressed(mb_left)) {
			...
			if (point_in_rectangle(mouse_x, mouse_y, room_width - 368, 400, room_width - 48, 576)) {
				if (special == "s booster") {
					...
					collision_circle_list(x, y, range, tower_obj, false, true, targetable, false)
				}
				if (special == "s commander") {
