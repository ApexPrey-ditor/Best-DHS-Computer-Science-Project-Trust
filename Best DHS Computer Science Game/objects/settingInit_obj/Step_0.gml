if (room != mainMenu) {
	if (not global.paused) {
		...
		if (mouse_check_button_pressed(mb_left) and not placing and not global.upgradeMenu and room != radarRoom) {
				...
				for (var w = 0; w < 2; w++) {
					...
					if (point_in_rectangle(mouse_x, mouse_y, room_width - 336 + w * 160, 240 + i * 160, room_width - 208 + w * 160, 368 + i * 160) and global.money >= ceil(costs[page * 8 + i * 2 + w] * global.modEffects[11]) and global.towers < ceil(global.modEffects[16]) and array_contains(global.availableTowers, page * 8 + i * 2 + w)) {
						switch (page * 8 + i * 2 + w) {
							...
							case 5:
								instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
								{damage : 0.2, aoe : 1, fireSpeed : 30, range : 150, detections : [false, true, false], spread : 10, type : 3, projSpeed : 5, lifetime : 60, special : "flame", effect : [1/20, 200], type : 3, cost : ceil(costs[page * 8 + i * 2 + w] * global.modEffects[11]), towerType : page * 8 + i * 2 + w})
								placing = true
								break;
							case 6:
								instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
							...
							case 9:
								instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
								{damage : 0, fireSpeed : 0, range : 250, effect: [100], special : "money", cost : ceil(costs[page * 8 + i * 2 + w] * global.modEffects[11]), towerType : page * 8 + i * 2 + w})
								placing = true
								break;
							case 10:
								instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
	...
	if (global.health <= 0) {
		...
		if (global.paused == false) {
			for(var i = 1; i < global.stage; i++) {
				global.exp += i * 5
			}
		}
		global.paused = true
		pauseMenu_obj.goUp = false
		while (global.exp > floor((1.1 ^ global.expLevel) * 100)) {
			global.exp -= floor((1.1 ^ global.expLevel) * 100)
			global.expLevel++
			global.skillPoints++
		}
