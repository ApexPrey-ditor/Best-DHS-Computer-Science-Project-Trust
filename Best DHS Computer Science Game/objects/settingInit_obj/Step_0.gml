		...
		for (var w = 0; w < 2; w++) {
			...
			if (point_in_rectangle(mouse_x, mouse_y, room_width - 336 + w * 160, 240 + i * 160, room_width - 208 + w * 160, 368 + i * 160) and global.money >= ceil(costs[page * 8 + i * 2 + w] * global.modEffects[11]) and global.towers < ceil(global.modEffects[16])) {
				switch (page * 8 + i * 2 + w) {
					...
					case 5:
						instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
						{damage : 0.5, fireSpeed : 30, range : 150, detections : [false, true, false], spread : 10, type : 3, projSpeed : 5, lifetime : 60, special : "flame", effect : [1/20, 200], type : 3, cost : ceil(costs[page * 8 + i * 2 + w] * global.modEffects[11]), towerType : page * 8 + i * 2 + w})
						placing = true
						break;
					case 6:
						instance_create_depth(mouse_x, mouse_y, 0, tower_obj,
