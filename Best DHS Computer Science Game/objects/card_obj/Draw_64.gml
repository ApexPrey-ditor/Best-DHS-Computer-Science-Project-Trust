if (selecting) {
	...
	for (var i = 0; i < sprite_get_number(towerIcons256_spr) - 1; i++) {
		if (i < 12) {
			if (array_contains(appliable, i) and (array_length(global.upgrades[i]) < 3 or repeatable) and array_contains(global.availableTowers, i)) {
			...
			if (array_contains(appliable, i) and (array_length(global.upgrades[i]) < 3 or repeatable) and array_contains(global.availableTowers, i)) {
				...
				draw_sprite_ext(towerIcons256_spr, i, room_width / 2 - 408 + (i - 12) * 272, room_height / 2 + 280, 1, 1, 0, c_black, 1)
			}
		}
	}
}
