...
if (room == intermission) {
	draw_set_colour(c_green)
	draw_circle(room_width / 2 - 192, room_height / 2, 8, false)
	
	draw_line_width(room_width / 2 - 192, room_height / 2, room_width / 2 + dcos(radarDir) * 512 - 192, room_height / 2 + -dsin(radarDir) * 512, 10)
	
	for (var i = 0; i < 30; i++) {
		draw_set_alpha((31 - i) / 62)
		draw_line_width(room_width / 2 - 192, room_height / 2, room_width / 2 + dcos(radarDir + i) * 512 - 192, room_height / 2 + -dsin(radarDir + i) * 512, 10)
	}
	
	draw_set_alpha(0.15)
	draw_set_circle_precision(40)
	draw_circle(room_width / 2 - 192, room_height / 2, 512, false)
	
	for (var i = -7; i < 8; i++) {
		draw_line(room_width / 2 + (i * 64) - 192, room_height / 2 + sqrt(262144 - power(i * 64, 2)), room_width / 2 + (i * 64) - 192, room_height / 2 - sqrt(262144 - power(i * 64, 2)))
		draw_line(room_width / 2 + sqrt(262144 - power(i * 64, 2)) - 192, room_height / 2 + (i * 64), room_width / 2 - sqrt(262144 - power(i * 64, 2)) - 192, room_height / 2 + (i * 64))
	}
	
	draw_set_alpha(1)
	
	draw_set_circle_precision(28)
	draw_circle(room_width / 2 - 192, room_height / 2, 128, true)
	draw_set_circle_precision(32)
	draw_circle(room_width / 2 - 192, room_height / 2, 256, true)
	draw_set_circle_precision(36)
	draw_circle(room_width / 2 - 192, room_height / 2, 384, true)
	draw_set_circle_precision(40)
	draw_circle(room_width / 2 - 192, room_height / 2, 512, true)
