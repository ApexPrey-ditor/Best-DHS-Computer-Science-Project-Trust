...
switch (image_index) {
	...
	// Piercing Rounds
	case 4:
		stats = {display : "Piercing Rounds", desc : "Gives solid hitting", d1 : true}
		appliable = global.oneTimesValues[array_get_index(global.oneTimesKey, image_index)]
		repeatable = false
		oneTime = true
		break;
	...
	// Lose Yourself
	case 7:
		stats = {display : "Lose Yourself", desc : "+|% Damage\n -|% Fire Rate", damage : 1, fireSpeed : -0.45}
		replace = [100, 45]
	...
	case 8:
		stats = {display : "gdrrrrrrrrrrr brrrrrrrrrrat brrrrrrrrrat brrrrrrrrrrrrrrrrrrrrrrrrrrr wrrrrrrrrrrrrrrrrriririirir", desc : "+|% Fire Rate\n-|% damage", damage : -0.5, fireSpeed : 1}
		replace = [100, 50]
		appliable = [0, 1, 2, 3]
		repeatable = false
		break;
	// Triangle Bullet
	case 9:
		stats = {display : "Triangle Bullet", desc : "+| Pierce", pierceL : 2}
		replace = [2]
		appliable = [0, 1, 2, 3]
		repeatable = false
		break;
	// HELP IT BURNS
	case 10:
		stats = {display : "HELP IT BURNS", desc : "Attacks burn enemies", burn : true}
		appliable = global.oneTimesValues[array_get_index(global.oneTimesKey, image_index)]
		repeatable = false
		oneTime = true
		break;
	...
	// Bouncing Shot
	case 12:
		stats = {display : "Bouncing Shot", desc : "+| bullet bounces \n-|% damage", damage : -0.5, bounce : 2}
		replace = [2, 50]
		appliable = [0, 1, 2, 3]
		repeatable = false
		break;
	...
	case 13:
		stats = {display : "Sodium Hydrogen Oxygen Malevolent Bond", desc : "+| Pierce+ \n+|% damage", pierceL : 1, damage : 0.15}
		replace = [1, 15]
		appliable = [0, 1, 2, 3]
		repeatable = false
		break;
	...
	case 14:
		stats = {display : "Sodium Hydrogen Oxygen Malevolent Bond", desc : "Sets things on cold", slow : 1}
		replace = [1, 15]
		appliable = [0, 1, 2, 3]
		repeatable = false
		break;
	...
	case 17:
		stats = {display : "Pentaerythritol Tetranitrate", desc : "+|% AOE \n+| pierce \n-|% damage", damage : -0.25, AOE : 0.5, pierceL : 3}
		replace = [50, 3, 25]
		appliable = [4, 5, 6, 7]
		repeatable = false
		break;
	...
	case 18:
		stats = {display : "worlds brightest flashlight", desc : "removes camo from enemies every 3rd shot \n-|% attack speed", fireSpeed : -0.5, decamo : true}
		replace = [50]
	...
	// hydrogen
	case 19:
		stats = {display : "hydrogen", desc : "Applies burning effect \n-|% attack speed \n -|% AOE", fireSpeed : -0.25, AOE : -0.1, burn : true}
		replace = [25, 10]
		appliable = global.oneTimesValues[array_get_index(global.oneTimesKey, image_index)]
		repeatable = false
		oneTime = true
	...
	case 20:
		stats = {display : "Hey Guys I Think There's a Fly Biting Me", desc : "+|% AOE \n-|% damage", damage : -0.5, AOE : 1}
		replace = [100, 50]
		appliable = [4, 5, 6, 7]
		repeatable = false
		break;
	// Whats a Physics?
	case 21:
		stats = {display : "Whats a Physics?", desc : "+| pierce", pierceL : 3}
		replace = [3]
		appliable = [4, 5, 6, 7]
		repeatable = false
		break;
	...
	// sharp explosions
	case 23:
		stats = {display : "sharp explosions", desc : "+|% damage \n-|% pierce", damage : 1.25, pierce : -0.75}
		replace = [125, 75]
		appliable = [4, 5, 6, 7]
		repeatable = false
		break;
	...
	// DESTROY
	case 25:
		stats = {display : "DESTROY", desc : "Hit is delayed by | seconds \n+|% Damage \n+|% Attack Speed", fireSpeed : 0.05, damage : 0.25, delay : 30}
		replace = [0.5, 25, 5]
		appliable = [4, 5, 6, 7]
		repeatable = false
		break;
	...
	// Psychiatrist
	case 42:
		stats = {display : "Psychiatrist", desc : "+|% effects to towers with Schizophrenia \n Or if moneymaker \n+10% money for each tower with Schizophrenia", psychiatrist : 1}
		replace = [50, 10]
		appliable = [8, 9, 10, 11]
		repeatable = false
		break;
	// eyes on the prize
	case 43:
		stats = {display : "eyes on the prize", desc : "every support tower makes money based on their cost \n-|% nerf to effect", eyesOnThePrize : 1, buff : -0.8}
		replace = [80]
		appliable = [8, 10, 11]
		repeatable = false
		break;
	// pocketer
	case 44:
		stats = {display : "pocketer", desc : "+|% more effect \n+|% less range", buff : 0.4, range : -0.5}
		replace = [40, 50]
		appliable = [8, 10, 11]
		repeatable = false
		break;
	// big eyes
	case 45:
		stats = {display : "big eyes", desc : "|% range \n-|% effect", buff : -0.1, range : 0.5}
		replace = [50, 10]
		appliable = [8, 10, 11]
		repeatable = false
		break;
	...
	case 46:
		stats = {display : "i dont have very many beans so you guys are gonna have to share", desc : "+|% effect \n-|% effect for every tower in range", buff : 0.5, notManyBeans : 1}
		replace = [50, 10]
		appliable = [8, 9, 10, 11]
		repeatable = false
		break;
	...
	case 47:
		stats = {display : "chinese sweatshop conditions", desc : "-|% effect nerf \n+|% more effective per tower in range", buff : -0.8, sweatshop : 1}
		replace = [80, 10]
		appliable = [8, 9, 10, 11]
		repeatable = false
		break;
	// lesbian
	case 48:
		stats = {display : "lesbian", desc : "if there are multiple instances of the same tower of lesbian support tower: +|% effects", lesbian : 0.05}
		replace = [5]
		appliable = [8, 9, 10, 11]
		repeatable = false
		break;
	...
	// child support
	case 50:
		stats = {display : "child support", desc : "gives a |% discount on upgrades \n-| lives every time this upgrade is applied", childSupport : 1}
		replace = [10, 10]
		appliable = [8, 10, 11]
		repeatable = false
		break;
	// addiction
	case 51:
		stats = {display : "addiction", desc : "Effects stack and apply additively \nIf a tower with addiction isn't bought before round 30, no addiction towers can be bought \n+|% effect power \nafter 5 rounds tower effect turn negative \nTower Cannot Be Sold", buff : 1, addiction : true}
		replace = [100]
		appliable = [8, 10]
		repeatable = false
		break;
	...
	// You don't know JACK
	case 53:
		stats = {display : "You don't know JACK", desc : "-|% effect power \n+|% damage \n+|% attack speed", buff : -0.5, fireSpeed : 0.2, damage : 0.25}
		replace = [50, 25, 20]
		appliable = [12, 13, 14]
		repeatable = false
		break;
	// Teh 11 of Spades
	case 54:
		stats = {display : "Teh 11 of Spades", desc: "+|% effect power \n-|% damage \n-|% attack speed", buff : 0.5, fireSpeed : -0.2, damage : -0.25}
		replace = [50, 25, 20]
		appliable = [12, 13, 14]
		repeatable = false
		break;
